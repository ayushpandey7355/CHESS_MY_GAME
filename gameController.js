...
function movePiece(piece, startingPosition, endingPosition) {
    ...
    if (boardPiece != '.') {
        // Ensure that the correct player's piece is being moved
        if ((boardPiece === boardPiece.toUpperCase() && curPlayer == 'black') ||
            (boardPiece === boardPiece.toLowerCase() && curPlayer == 'white')) {
            ...
            // Handle pawn promotion if a pawn reaches the last rank
            if (boardPiece.toLowerCase() === 'p' && (endingPosition[0] === 0 || endingPosition[0] === 7)) {
                boardPiece = curPlayer === 'white' ? 'q' : 'Q'; // Promote to queen (you could add other options)
            }

            curBoard[endingPosition[0]][endingPosition[1]] = boardPiece;

            // Move the piece visually on the board
            const destinationSquare = document.getElementById(`${endingPosition[0] + 1}${endingPosition[1] + 1}`);
            destinationSquare.textContent = '';
            destinationSquare.appendChild(piece);
            ...
            if (isCheckmate()) {
                alert(`${curPlayer} wins by checkmate!`);
            }
...
// Validate whether the move results in a check or checkmate
function isCheckmate() {
    // Check if the king is in check
    const kingPosition = findKing(curPlayer);
    if (isKingInCheck(kingPosition)) {
        // If the king is in check, verify if there are any valid moves left to escape check
        if (!hasValidMoves(curPlayer)) {
    ...
    for (let i = 0; i < 8; i++) {
        for (let j = 0; j < 8; j++) {
            ...
            if (piece != '.' && isEnemyPiece(piece)) {
                if (validateMovement([i, j], kingPosition)) {
...
// Check if any valid moves are left for the player
function hasValidMoves(player) {
    ...
    return false;  // No valid moves left, it's checkmate
}

// Ensure that only enemy pieces can attack the king
function isEnemyPiece(piece) {
    return (piece === piece.toUpperCase() && curPlayer === 'white') || 
           (piece === piece.toLowerCase() && curPlayer === 'black');
}

// Implement castling logic (simplified)
function canCastle() {
    // Check if the king and rook have not moved, and the path between them is clear
    // Additional logic to handle castling (both kingside and queenside)
}

// Add en passant logic
function canEnPassant() {
    // Logic to check if an en passant move is possible
}

// Validate pawn promotion
function promotePawn(pawnPosition, color) {
    // Logic to handle promotion to a queen, rook, bishop, or knight
}
